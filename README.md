...
**Дати моделі контекст** (регіон, тип книги):

```bash
./indexer-v0.2-linux-x86_64 my_project /шлях/до/сканів --description "Волинь, метричні книги"
```
...
| `--description` | — | Текст-підказка для моделі (регіон, тип документів) |
...
Текст у **`--description`** може бути довільним поясненням для моделі або коротким ім’ям файлу з папки `prompts/` репозиторію (без `.txt`) — [детальніше](docs/usage.md).
