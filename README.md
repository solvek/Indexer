...
Програма проходить папку зі сканами (фото сторінок метричних книг, актів тощо), відправляє їх у AI-модель (Gemini за замовчуванням, OpenAI/ChatGPT-моделі опційно) і зберігає знайдені прізвища, імена та роки в **локальну базу** на вашому комп’ютері. Є готові файли під Windows, Linux і macOS.
...
2. **Ключ API для AI-моделі**: Gemini — [aistudio.google.com/apikey](https://aistudio.google.com/apikey), OpenAI — [platform.openai.com/api-keys](https://platform.openai.com/api-keys).
...
Далі відкрийте файл `.env` і в рядку **`GEMINI_API_KEY=`** після знака `=` вставте свій Gemini-ключ (без лапок і пробілів). Для OpenAI змініть **`AI_PROVIDER=openai`** і заповніть **`OPENAI_API_KEY=`**. Решту рядків для першого разу можна не змінювати.
...
| `--provider` | з `.env` або `gemini` | AI-провайдер: `gemini` або `openai` |
| `--model` | з `.env` або типова для провайдера | Назва моделі провайдера, наприклад `gemini-2.0-flash-lite` або `gpt-4o-mini` |
