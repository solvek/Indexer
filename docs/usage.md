...
| `--provider` | optional | з `.env` або `gemini` | AI-провайдер: `gemini` або `openai` |
| `--model` | optional | з `.env` або типова для провайдера | Назва моделі провайдера |
...
| `--request-delay` | optional | `0` | Пауза в секундах після кожного звернення до моделі |
